...
export default function ProductCard({ product }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
      <Link href={`/product/${product.id}`}><img src={product.image} alt={product.name} style={{ width: '100%', height: '200px', objectFit: 'cover' }} /></Link>
      <div style={{ padding: '1rem' }}>
        ...
        <button onClick={() => addItem(product)} style={{ width: '100%', padding: '0.6rem', background: '#059669', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>افزودن به سبد خرید</button>
      </div>
    </div>
  );
}
