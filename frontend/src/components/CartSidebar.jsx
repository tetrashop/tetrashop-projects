  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-white w-full max-w-md h-full shadow-xl p-6 flex flex-col overflow-y-auto">
        <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold">سبد خرید</h2><button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-2xl">&times;</button></div>
        {items.length === 0 ? <p className="text-gray-500 text-center">سبد خرید خالی است</p> : (
          <>
            <ul className="flex-1 divide-y">
              {items.map(item => (
                <li key={item.product.id} className="py-4 flex gap-4 items-center">
                  ...
                  <div className="flex-1"><h4 className="font-semibold">{item.product.name}</h4><p className="text-sm text-gray-500">{formatPrice(item.product.price)} تومان</p>
            ...
            <div className="border-t pt-4 mt-4"><div className="flex justify-between text-xl font-bold"><span>جمع:</span><span>{formatPrice(totalPrice)} تومان</span></div>
