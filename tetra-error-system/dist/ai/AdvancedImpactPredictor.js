...
exports.AdvancedImpactPredictor = void 0;
const ErrorHistoryService_1 = require("../services/ErrorHistoryService");
class AdvancedImpactPredictor {
    static async predictImpact(factors) {
        // محاسبه هر عامل به صورت موازی
        const [errorFrequencyScore, businessCriticalityScore, timeSensitivityScore, userImpactScore] = await Promise.all([
        ...
        const finalScore = (errorFrequencyScore * this.FACTOR_WEIGHTS.errorFrequency) +
        ...
        const recommendations = this.generateRecommendations(finalScore, factors.errorCode, factors.operationName, estimatedMTTR);
        // محاسبه اطمینان پیش‌بینی
        const confidence = this.calculateConfidence(errorFrequencyScore, factors.errorCode);
        return {
    ...
    static async calculateErrorFrequencyScore(errorCode, operation) {
        try {
            const history = await ErrorHistoryService_1.ErrorHistoryService.getErrorStats(errorCode, operation);
            ...
            if (occurrenceRate > 5 && resolutionRate < 0.3)
                return 0.9;
            if (occurrenceRate > 3)
                return 0.7;
            if (occurrenceRate > 1)
    ...
    static calculateBusinessCriticalityScore(operation) {
        if (this.CRITICAL_OPERATIONS.includes(operation))
            return 0.9;
        if (this.IMPORTANT_OPERATIONS.includes(operation))
            return 0.6;
        return 0.3; // عملیات عادی
    }
    static calculateTimeSensitivityScore(timestamp) {
        ...
        if (isPeakHour && isWorkday)
            return 0.9;
        if (isPeakHour || isWorkday)
            return 0.6;
        return 0.3; // آخر هفته یا ساعات غیراوج
    }
    static calculateUserImpactScore(serviceTier) {
        switch (serviceTier) {
    ...
    static estimateMTTR(score, errorCode) {
    ...
    static generateRecommendations(score, errorCode, operationName, estimatedMTTR) {
        const recommendations = [];
        // توصیه‌های بر اساس نمره
        if (score >= 0.8) {
            recommendations.push('🚨 **هشدار فوری**: تیم عملیاتی را در جریان قرار دهید', `⏱️ **اولویت بالا**: حداکثر تا ${estimatedMTTR} دقیقه آینده بررسی شود`, '📊 **نظارت شدید**: سلامت سرویس‌های وابسته را پیوسته بررسی کنید');
        }
        else if (score >= 0.6) {
            recommendations.push('👨‍💻 **تیم فنی**: مهندس ارشد را مطلع کنید', '📝 **مستندسازی**: لاگ‌های کامل را برای تحلیل آتی جمع‌آوری کنید', `⏳ **زمان‌بندی**: در ${estimatedMTTR} دقیقه آینده رسیدگی شود`);
        }
        else {
            recommendations.push('✅ **اولویت عادی**: در صف بررسی معمولی قرار گیرد', '📈 **گزارش‌دهی**: در گزارش دوره‌ای خطاها ثبت شود', '🔍 **بررسی سطحی**: علت اصلی در فرصت مناسب بررسی شود');
        }
    ...
    static calculateConfidence(frequencyScore, errorCode) {
        // اطمینان بر اساس تجربه تاریخی
        let confidence = 0.5; // پیش‌فرض
        if (frequencyScore > 0.7)
            confidence = 0.9; // خطای کاملاً شناخته شده
        else if (frequencyScore > 0.4)
            confidence = 0.7; // خطای نسبتاً شناخته شده
...
exports.AdvancedImpactPredictor = AdvancedImpactPredictor;
// عوامل وزندهی برای پیش‌بینی
AdvancedImpactPredictor.FACTOR_WEIGHTS = {
