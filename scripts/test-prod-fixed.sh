...
echo "📊 تست بهره‌وری TetraSaaS - نسخه اصلاح شده"
echo "=========================================="
echo "تاریخ تست: $(date '+%Y/%m/%d %H:%M:%S')"
echo ""
...
echo "🏆 رتبه: $(get_grade $health_percentage $avg_response_time)"
...
if [ $health_percentage -lt 50 ]; then
    echo "  • راه‌اندازی سرویس‌های غیرفعال: ~/tetra-services-manager.sh start"
    ...
    echo "  • افزایش پایداری سرویس‌ها"
    echo "  • مانیتورینگ مداوم"
    echo "  • تست دوره‌ای"
else
    echo "  • حفظ وضعیت فعلی"
...
# ذخیره نتایج
RESULTS_FILE="productivity-$(date +%Y%m%d-%H%M%S).json"
...
echo ""
echo "📁 نتایج ذخیره شد در: $RESULTS_FILE"

# مقایسه با قبلی (اگر وجود دارد)
PREVIOUS_FILE=$(ls -t productivity-*.json 2>/dev/null | head -2 | tail -1)
if [ -n "$PREVIOUS_FILE" ] && [ "$PREVIOUS_FILE" != "$RESULTS_FILE" ]; then
    echo ""
    echo "📅 مقایسه با تست قبلی:"
    
    # خواندن داده‌های قبلی (ساده شده)
    if [ -f "$PREVIOUS_FILE" ]; then
        prev_health=$(grep -o '"health_percentage":[0-9]*' "$PREVIOUS_FILE" | cut -d: -f2 | head -1 || echo "0")
        prev_response=$(grep -o '"avg_response_time_ms":[0-9]*' "$PREVIOUS_FILE" | cut -d: -f2 | head -1 || echo "0")
        prev_score=$(grep -o '"productivity_score":[0-9]*' "$PREVIOUS_FILE" | cut -d: -f2 | head -1 || echo "0")
        
        health_change=$((health_percentage - prev_health))
        response_change=$((prev_response - avg_response_time))
        score_change=$((productivity_score - prev_score))
        
        echo "  سلامت: ${prev_health}% → ${health_percentage}% (تغییر: ${health_change}%)"
        echo "  زمان پاسخ: ${prev_response}ms → ${avg_response_time}ms (تغییر: ${response_change}ms)"
        echo "  نمره: ${prev_score} → ${productivity_score} (تغییر: ${score_change})"
        
        if [ $score_change -gt 0 ]; then
            echo "  📈 بهبود: +${score_change} امتیاز"
        elif [ $score_change -lt 0 ]; then
            echo "  📉 کاهش: ${score_change} امتیاز"
        else
            echo "  ➡️ بدون تغییر"
        fi
    fi
